...
const PersonalFooter = () => {
  ...
  const socialLinks = [
    ...
    { name: 'LinkedIn', icon: <Linkedin className="h-5 w-5" />, url: 'www.linkedin.com/in/naman-ghevariya' },
   
    { name: 'Email', icon: <Mail className="h-5 w-5" />, url: 'mailto:naman@rakshastack.com' },
  ];
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <Link to="/" onClick={scrollToTop} className="flex items-center space-x-3.5 mb-4.5 group">
              { <motion.img 
                ...
                alt="Naman Ghevariya Logo" 
                className="h-12 rounded-xl w-12"
                whileHover={{ scale: 1.08, rotate: 3 }}
                transition={{type: "spring", stiffness: 250}}
              /> }
              <span className="text-2xl font-bold text-gradient-personal group-hover:brightness-115 transition-all duration-200">Naman Ghevariya</span>
            </Link>
        ...
        >
          <p className="text-sm">
            &copy; {new Date().getFullYear()} Naman Ghevariya. All rights reserved.
          </p>
